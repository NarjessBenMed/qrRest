...
router.put('/update-menu', isAuth, authRole(ROLE.OWNER), async (req, res, next) => {
  try {
    const { restaurant, name, nameRest, logo, price, description, categorie } = req.body;
    if (!req.file) {
...
router.put('/delete-item', isAuth, authRole(ROLE.OWNER), async (req, res, next) => {
  try {
    const { idMenu, id } = req.body;
    console.log('ids', idMenu, id);
    const foundFood = await Menu.findOne({ _id: idMenu });
    console.log('food', foundFood);
...
router.put('/edit-item', isAuth, authRole(ROLE.OWNER), async (req, res, next) => {
  try {
    let image;
